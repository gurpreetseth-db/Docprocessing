...
# MAGIC Creates users table and 20 sample PDF documents.
# MAGIC Expects catalog, schemas, and volume to already exist (created by bundle deployment).

# COMMAND ----------

# MAGIC %md
# MAGIC ## Parameters (passed from job task)

# COMMAND ----------

dbutils.widgets.text("catalog", "DocProcessing", "Catalog Name")
dbutils.widgets.text("bronze_schema", "DocProcess_Bronze", "Bronze Schema")
dbutils.widgets.text("volume_name", "InputPDFs", "Volume Name")
...
print(f"Using catalog: {catalog}, schema: {bronze_schema}, volume: {volume_path}")
...
# Re-read widgets after %restart_python
catalog = dbutils.widgets.get("catalog")
bronze_schema = dbutils.widgets.get("bronze_schema")
volume_name = dbutils.widgets.get("volume_name")
volume_path = f"/Volumes/{catalog}/{bronze_schema}/{volume_name}"
...
# MAGIC %md
# MAGIC ## Step 1: Create Users Table
...
df_users.write.mode("overwrite").saveAsTable("users")
